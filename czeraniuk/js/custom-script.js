		...
		function center_logo(){
			var window_h = $(window).height(),
					 logo_h = $('.navbar-brand').height(),
					 logo_top = window_h/2 - logo_h/2;
			
			console.log(window_h)
			
			$('.navbar-brand').css({marginTop:logo_top, display:'block'});
...
$(document).ready(function(){
	...
	/*-----------------------------------------*/
		...
		if( $('#map').length > 0 || $('#mobile-map').length > 0 ){
	
			// init the map and create a cluster with some markers
	    $('#map, #mobile-map').gmap3({
	      map:{
	        options:{
	          center:[-27.3285801,-55.8622411],
	          zoom: 17,
	          panControl: false,
