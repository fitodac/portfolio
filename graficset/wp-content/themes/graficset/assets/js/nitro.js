	...
	$.fn.tilt = function(options){
		...
		this.each(function(){
			...
			(opt.wrapper).hover(function(_event){
				...
				(opt.wrapper).find(opt.item).each(function(_index, _el){
			...
			function setTransform(obj, x, y, posX, posY){
				...
				if( $(obj).data('offset') )
					var _translateZ = parseInt( $(obj).data('offset') );
				else
					var _translateZ = 0;


				if( $(obj).data('scale') )
					var _scale = $(obj).data('scale');
				else
					var _scale = opt.scale;
				...
				if( opt.perspective3D ){
					$(obj).css({transform: 'scale('+ _scale +') translateZ('+ _translateZ +'px) translate('+ posX +'px, '+ posY +'px) rotateY('+ x +'deg) rotateX('+ y * -1 +'deg)'});
				}else{
					$(obj).css({transform: 'scale('+ _scale +') translateZ('+ _translateZ +'px) rotateY('+ x +'deg) rotateX('+ y * -1 +'deg)'});
				}//if
			...
			// Destroy
			if( options === 'destroy' ){
				var _newObj = _this.clone();
				_this.replaceWith(_newObj);
			}
	...
	$.fn.offCanvas = function(options){
		return this.each(function(){
			...
			if( _this.hasClass('off-canvas-hover') )
				_sidebar.on('mouseover', function(){
					_this.addClass('in');
				})
