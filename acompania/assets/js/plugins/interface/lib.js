...
var Vimeo = {
    ...
    attach: function () {
        var self = this,
            ...
            structure = [
            '<div class="featured">',
                '<iframe src="https://player.vimeo.com/video/'+main+'" width="960" height="540" frameborder="0" webkitAllowFullScreen mozallowfullscreen allowFullScreen />',
            '</div>',
    ...
    fetch: function () {
        var self     = this,
            videos   = data.videos,
            carousel = jQuery('.carousel-container'),
            i, l;

        for (i = 0, l = videos.length; i < l; i += 1) {
            var id = self.getId(videos[i].url);

            jQuery.getJSON('https://www.vimeo.com/api/v2/video/' + id + '.json?callback=?', {format: "json"}).done(function (data) {
                var url   = data[0].url,
    ...
    carousel: function () {
        var self      = this,
            ...
            controll  = jQuery('.carousel-container .controll'),
            featured  = jQuery('.featured'),
            photosLen = slider.find('li').length;

        if (photosLen > 3) {
            var itemWidth = jQuery('.slider li').outerWidth(true);
            ...
            jQuery(self.container).on('click', '.carousel-container .controll', function (){
                ...
                if ($this.hasClass('next')) {
                    ...
                    jQuery('.slider:not(:animated)').animate({'left' : leftIndent}, 500, function () {
        ...
        jQuery(self.container).on('click', '.thumb', function (e) {
            ...
            featured.find('iframe').attr({'src' : 'https://player.vimeo.com/video/'+self.getId(href)+'?autoplay=1'});
